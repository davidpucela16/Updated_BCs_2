...
if alpha==50:
    q_COMSOL_sources=np.array([[0.5121,0.5502,0.5967,0.6322,0.6515],[0.509,0.5459,0.5927,0.6437,0.7048]]).T
    q_COMSOL_sink=np.array([[-0.1828,-0.134,-0.0713,-0.0298,-0.0094],[0.6917,0.6798,0.664,0.6735,0.7142]]).T
...
for i in array_of_pos:
    ...
    if c==0 or c==4:
        sep=int(d[c])
        file='../' + 'Double_source_COMSOL/Double_source_same/Double_source_alpha' +  str(alpha) + '_2D_d' + str(sep) + '_sources_same.txt'
...
for i in array_of_pos:
    ...
    if c==0 or c==4:
        sep=int(d[c])
        file='../' + 'Double_source_COMSOL/Double_source_same/Double_source_alpha' +  str(alpha) + '_2D_d' + str(sep) + '_SourceSink_same.txt'
        df=pandas.read_fwf(file)
        ref_data=np.array(df).T #reference 2D data from COMSOL
        phi_bar_COMSOL=np.max(ref_data[2])
        r.set_up_manual_reconstruction_space(ref_data[0], ref_data[1])
...
axs[0,0].plot(d, array_code, label='MyCode')
axs[0,0].plot(d, array_COMSOL, label='COMSOL')
axs[0,0].legend()
axs[0,0].set_xlabel("$distance/R_v$")
axs[0,0].set_ylabel("q")
axs[0,0].set_title("$q_1 source (Up right)$")

axs[0,1].plot(d,np.abs(array_code-array_COMSOL)/array_COMSOL)
axs[0,1].set_xlabel("$distance/R_v$")
axs[0,1].set_ylabel("rel error")
axs[0,1].set_title("relative error")


axs[0,2].plot(d,np.abs(array_code-array_COMSOL))
...
axs[1,0].plot(d, array_code, label='MyCode')
axs[1,0].plot(d, array_COMSOL, label='COMSOL')
axs[1,0].legend()
axs[1,0].set_xlabel("$distance/R_v$")
axs[1,0].set_ylabel("q")
axs[1,0].set_title("$q_2 (source)$")

axs[1,1].plot(d,np.abs(array_code-array_COMSOL)/np.abs(array_COMSOL))
axs[1,1].set_xlabel("$distance/R_v$")
axs[1,1].set_ylabel("rel error")
axs[1,1].set_title("relative error")


axs[1,2].plot(d,np.abs(array_code-array_COMSOL))
