...
import os
os.chdir(directory)
directory_script='/home/pdavid/Bureau/Code/SS_auto57/2D_cartesian/Updated_BCs/Figures_and_Tests/Double_source'
...
cells=5
h_coarse=L/cells
#ratio=int(np.max((h_coarse/0.1,6)))
ratio=20
#Rv=np.exp(-2*np.pi)*h_coarse
...
#%%
directory_files=directory_script + '/Double_source_COMSOL/Double_source_diff/alpha' + str(alpha)
if sources:
...
if diff_radii:
    for k in range(len(d)):
        ...
        r.solve_linear_prob(BC_type, BC_value, C_v_array)
        phi_FV=r.s_FV #values on the FV cells
        phi_q=r.q #values of the flux
        ...
        r.reconstruction_manual(C_v_array)
        r.reconstruction_boundaries(BC_value, BC_type)
        phi_MyCode=r.s+r.DL+r.SL
        ...
        r.reconstruction_manual(C_v_array)
        r.reconstruction_boundaries(BC_value, BC_type)
        phi_MyCode_1D=r.s+r.DL+r.SL
        
        fig, axs=plt.subplots(2,3, figsize=(16,8))
        
        col=[  'pink','c', 'blue']
        side=(directness+0.5)*h_coarse*2
        vline=(y_coarse[1:]+x_coarse[:-1])/2
